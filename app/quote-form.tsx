...
export default function QuoteForm({lang,items,setItems,whatsapp,salesEmail}:{lang:Lang;items:QuoteItem[];setItems:Dispatch<SetStateAction<QuoteItem[]>>;whatsapp:string;salesEmail:string}) {
  ...
  const inquiryText=()=>inquiry?[
    ...
    ...inquiry.items.flatMap(item=>{const p=products.find(p=>p.id===item.productId)!;const query=new URLSearchParams({variant:item.variantId,unit:item.unit});if(item.color)query.set('color',item.color);if(item.photo!==undefined)query.set('photo',String(item.photo));return [itemText(item,lang),`${origin}${productHref(p)}?${query}`,''];}),
