...
export default function Storefront({initialLang,whatsapp,salesEmail}:{initialLang:Lang;whatsapp:string;salesEmail:string}) {
  ...
  useEffect(()=>{
    ...
    document.title=lang==='id'?'Bos Bahan PVC | Anda Bosnya. Kami siapkan bahannya.':"Bos Bahan PVC | You’re the boss. We supply the materials.";
  ...
  const openChat=()=>{
    if(whatsapp)window.open(`https://wa.me/${whatsapp}?text=${encodeURIComponent(t('Halo Bos Bahan PVC, saya ingin berkonsultasi mengenai kebutuhan bahan untuk usaha saya.','Hello Bos Bahan PVC, I would like to discuss material requirements for my business.'))}`,'_blank','noopener,noreferrer');
  ...
  useEffect(()=>{
    ...
    try { void Promise.resolve(context.registerTool({name:'search_material_catalog',title:'Search the Bos Bahan PVC catalog',description:'Read material groups and available variants. Does not submit a quote or contact the business.',inputSchema:{type:'object',properties:{query:{type:'string'},language:{enum:['id','en']}},required:['query'],additionalProperties:false},annotations:{readOnlyHint:true},execute:(input:unknown)=>{
  ...
  return <>
    ...
    <header className="site-header"><div className="container header-inner">
      <a href="#home" className="wordmark" aria-label="Bos Bahan PVC"><img src="/images/logo-round.webp" alt="" width="57" height="57"/><div><strong className="brand-name">BOS <span>BAHAN PVC</span></strong></div></a>
    ...
    <main id="home">
      <section className="hero" aria-labelledby="hero-title">
        <img src="/images/warehouse.webp" alt={t('Fasilitas dan gudang material Bos Bahan PVC','Bos Bahan PVC material facility and warehouse')} className="hero-photo" width="1800" height="1350" fetchPriority="high"/>
        ...
        <div className="container hero-content"><div className="eyebrow text-gold hero-eyebrow">{t('Mitra Terpercaya untuk Pengadaan Bahan PVC','Your Trusted Partner for PVC Material Procurement')}</div>
          <div className="hero-heading-row"><h1 id="hero-title">{t('Anda Bosnya.','You’re the boss.')}<br/>{t('Kami siapkan','We supply')}<br/><span>{t('bahannya.','the materials.')}</span></h1><div className="hero-seal"><img src="/images/bos-approved-seal.webp" width="600" height="600" alt={t('Logo Bos Bahan PVC dengan lima bintang emas — BOS APPROVED','Bos Bahan PVC logo with five gold stars — BOS APPROVED')} fetchPriority="high"/></div></div>
          ...
          <div className="hero-photo-label"><span>BOS BAHAN PVC</span><p>{t('Dari gudang kami, untuk bisnis Anda.','From our warehouse to your business.')}</p></div>
        </div>
      </section>
      ...
      <section id="about" className="brand-section"><div className="container"><div className="grid md:grid-cols-[.85fr_1.15fr] gap-10 md:gap-16 items-center"><img className="brand-image" src="/images/brand-banner.webp" alt="Bos Bahan PVC — kulit jok motor mobil, mika, rigid, terpal, karpet" width="953" height="549" loading="lazy"/><div><div className="eyebrow text-[#927040]">{t('KENALI BOS BAHAN PVC','MEET BOS BAHAN PVC')}</div><h2 className="section-heading mt-4">{t('Anda menentukan arah.','You set the direction.')}<br/>{t('Kami siapkan pilihan.','We bring the options.')}</h2><p className="mt-5 leading-7 text-[#717972]">{t('Bagi kami, Anda adalah Bosnya. Kebutuhan bisnis Anda menjadi dasar setiap rekomendasi material. Bos Bahan PVC menghadirkan pilihan bahan untuk pelaku usaha otomotif, interior, kemasan, konveksi, dan perdagangan.','To us, you’re the boss. Your business requirements guide every material recommendation. Bos Bahan PVC brings together materials for automotive, interior, packaging, textile and trading businesses.')}</p></div></div><div className="process">{[
    ...
    {chatOpen&&<aside className="whatsapp-panel" aria-label="WhatsApp"><div className="bg-[#237b4d] text-white p-5 flex justify-between items-center"><div><strong>Bos Bahan PVC</strong><p className="text-xs mt-1 opacity-80">{t('Anda Bosnya, kami siap membantu.','You’re the boss. We’re here to help.')}</p></div><button aria-label={t('Tutup chat','Close chat')} onClick={()=>setChatOpen(false)}><X size={20}/></button></div><div className="p-5"><p className="text-sm leading-6 text-[#626e65]">{t('Layanan WhatsApp segera tersedia. Sementara itu, silakan sampaikan kebutuhan Anda melalui formulir penawaran.','WhatsApp service will be available soon. In the meantime, share your requirements using the quote form.')}</p><a className="btn btn-dark w-full mt-5" href="#quote" onClick={()=>setChatOpen(false)}>{t('Minta Penawaran','Request a Quote')}<ArrowRight size={16}/></a></div></aside>}
